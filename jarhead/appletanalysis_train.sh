...
if test $# -ne 2; then
    echo "Error: expected 2 parameters; $# provided"
    echo "Usage: $0 ANALYZER APPLET"
    exit 1
fi
...
elif [ "$DISASM_ANALYZER" = "$CURR/jarhead_train.py" ]; then
	
	result=`$DISASM_ANALYZER $TMP_DIR/$applet $CURR $TMP_DIR/disas`
	#echo $result >> ../train.arff
	echo $applet : $result >> ../train_log
	echo Done
